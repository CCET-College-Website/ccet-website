...
const CivilHod = () => {
  ...
  const fetchHodData = async () => {
    ...
    try {
      ...
      setError("Error loading HOD information. Please check the backend connection.");
    ...
    return (
        <SharedCivilLayout pageTitle="HOD Desk">
          <div className="bg-red-50 border border-red-200 rounded-lg p-6 text-center my-8">
            <p className="text-red-700">{error || "HOD data not found."}</p>
          </div>
        </SharedCivilLayout>
    );
  }

  const emails = hodData.email ? hodData.email.split('|').map(e => e.trim()) : [];
        ...
        <div className={styles.container}>
          ...
          <div className={styles.profileSection}>
            <div className={styles.profileContainer}>
              ...
              <img
                  ...
                  alt={hodData.name || "HOD Profile"}
                  onError={(e) => e.target.src = 'https://via.placeholder.com/200x250?text=HOD+Image'}
              />
            </div>
          ...
          <div className={styles.contentBox}>
            <div className={styles.message}>
              {parseMessage(hodData.description)}
            </div>
          </div>
        </div>
      </SharedCivilLayout>
