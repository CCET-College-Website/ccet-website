...
const OldEOA = () => {
    ...
    const TAB_ID = 41; // Old EOA Report tab ID
    ...
    const fetchPdfs = async () => {
        ...
        try {
            ...
            if (result.data && result.data.length > 0) {
                ...
                setError("No EOA reports available");
                setPdfs([]);
            }
        } catch (err) {
            setError("Error loading EOA reports");
            console.error("Old EOA Report fetch error:", err);
            setPdfs([]);
        } finally {
            setLoading(false);
        }
        ...
        return (
            <div className={styles.container}>
                ...
                <div className="flex justify-center items-center py-16">
                    <span className="text-gray-500">Loading EOA reports...</span>
                </div>
            </div>
        );
    }
    ...
    return (
        <div className={styles.container}>
            <h1 className={styles.heading}>
                {currentPdf?.pdf_name || 'EOA Report 2024-2025'}
            </h1>
            <div className={styles.underline}></div>
            ...
            <div className={styles.pdfPlaceholder}>
                <iframe
                    src={pdfUrl}
                    title={currentPdf?.pdf_name || 'EOA Report 2024-2025'}
            ...
            >
                <Download size={20} />
                Download {currentPdf?.pdf_name || 'EOA Report 2024-2025'}
            </button>
        </div>
    );
};
