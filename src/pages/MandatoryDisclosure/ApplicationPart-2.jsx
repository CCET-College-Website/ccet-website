...
const Parttwo = () => {
    ...
    const TAB_ID = 39; // Application Part-2 tab ID
    ...
    const fetchPdfs = async () => {
        ...
        try {
            ...
            if (result.data && result.data.length > 0) {
                ...
                setError("No application reports available");
                setPdfs([]);
            }
        } catch (err) {
            setError("Error loading application reports");
            console.error("Application Part-2 fetch error:", err);
            setPdfs([]);
        } finally {
            setLoading(false);
        }
    ...
    if (loading) {
        return (
            <div className={styles.container}>
                <h1 className={styles.heading}>Existing Institution Application Report Part 2 (2025-2026)</h1>
                <div className={styles.underline}></div>
                <div className="flex justify-center items-center py-16">
                    <span className="text-gray-500">Loading application reports...</span>
                </div>
            </div>
        );
    }
    ...
    return (
        <div className={styles.container}>
            <h1 className={styles.heading}>
                {currentPdf?.pdf_name || 'Existing Institution Application Report Part 2 (2025-2026)'}
            </h1>
            <div className={styles.underline}></div>
            ...
            <div className={styles.pdfPlaceholder}>
                <iframe
                    src={pdfUrl}
                    title={currentPdf?.pdf_name || 'Existing Institution Application Report Part 2'}
            ...
            >
                <Download size={20} />
                Download {currentPdf?.pdf_name || 'Report (Part 2) 2025-2026'}
            </button>
        </div>
    );
};
