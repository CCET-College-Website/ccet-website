    ...
    const fetchPdfs = async () => {
        ...
        try {
            ...
            if (result.data && result.data.length > 0) {
                ...
                setError("No FingerPrint issues available");
                setPdfs([]);
            }
        } catch (err) {
            setError("Error loading FingerPrint issues");
        ...
        return (
            <div className={styles.container}>
                ...
                <div className="flex justify-center items-center py-16">
                    <span className="text-gray-500">Loading FingerPrint issues...</span>
                </div>
            </div>
        );
    }
    ...
    return (
        <div className={styles.container}>
            <h1 className={styles.heading}>
                {currentPdf?.pdf_name || 'FingerPrint (Volume VII, Issue 2)'}
            </h1>
            <div className={styles.underline}></div>
            ...
            <div className={styles.pdfPlaceholder}>
                <iframe
                    src={pdfUrl}
                    title={currentPdf?.pdf_name || 'FingerPrint (Volume VII, Issue 2)'}
            ...
            >
                <Download size={20} />
                Download {currentPdf?.pdf_name || 'FingerPrint (Vol VII, Issue 2)'}
            </button>
        </div>
    );
};
